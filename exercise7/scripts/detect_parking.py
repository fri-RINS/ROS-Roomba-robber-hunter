...
def start_park_detection_callback(data):
    global can_start
...
def main():
    ...
    rate = rospy.Rate(1)
    start_park_detection_sub = rospy.Subscriber("start_park_detection", String, start_park_detection_callback, queue_size=10)
    while True:
        if can_start:
